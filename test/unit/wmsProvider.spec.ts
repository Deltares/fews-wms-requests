...
describe('WMS Provider Tests', function () {
  ...
  it('GetCapabilities', async function () {
    ...
    const provider = new WMSProvider(
      'https://rwsos-dataservices-ont.avi.deltares.nl/iwp/test/FewsWebServices/wms',
    )
    ...
    expect(res.layers[0]?.groupName).toBe(
      'Goddard NASA Monthly Historical Grids',
    )
    ...
    if (res.layers[0].keywordList && res.layers[0].keywordList.length > 0) {
      expect(res.layers[0].keywordList[0]['parameterId']).toBe(
        'T.hindcast.mean.global.historicalNat',
      )
    }
  ...
  it('GetCapabilitiesWithToken', async function () {
    fetchMock.route(
      'https://rwsos-dataservices-ont.avi.deltares.nl/iwp/test/FewsWebServicesSecured/wms?request=GetCapabilities&format=application%2Fjson',
      {
        status: 401,
        body: '',
      },
    ...
    const provider = new WMSProvider(
      'https://rwsos-dataservices-ont.avi.deltares.nl/iwp/test/FewsWebServicesSecured/wms',
